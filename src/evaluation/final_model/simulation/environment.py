...
class Env(gym.Env):
    ...
    def __init__(self, stock_data, agent_counts, global_seed=42, max_steps_per_episode=None, epo_run=False):
        super(Env, self).__init__()
        self.final_net_worth = None
        self.stock_data = stock_data
        self.max_steps_per_episode = len(stock_data)-1 if max_steps_per_episode is None else max_steps_per_episode
    ...
    def step(self, action):
        ...
        if self.current_step >= len(self.stock_data):
            print(f"⚠️ Reached end of stock data at step {self.current_step}!")
        ...
        for _ in range(num_stocks):
            decoded_actions.append(int(original_action % 3))  # Extract last digit (0=Hold, 1=Buy, 2=Sell)
            original_action //= 3  # Reduce action space for next stock
        ...
        # Execute the decoded actions in the simulation
    ...
    def save_trade_history(self):
        ...
        if not self.model.history:
            print(f"⚠️ No trade history recorded for Episode {self.episode_count + 1}!")
            return
        df = pd.DataFrame(self.model.history)
        filename = f"simulation/Trade_History/trade_history_ep{self.episode_count + 1}.csv"
    ...
    def calculate_trade_move_penalty(self):
        ...
        liquidity_penalty = -1.0 if agent.budget < 300 and agent.get_portfolio_value() > 20000 else 0.0
        ...
        penalty = np.clip(adjusted_penalty, -1.5, 0.0)
